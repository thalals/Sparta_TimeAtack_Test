...
@app.route('/article/<idx>/comment', methods=['PUT'])
def post_comment(idx):
    print('일로 오니?')
    article_idx = request.form.get('article_idx')
    comment = request.form.get('comment_give')

    db.article.update_one({'idx': int(article_idx)}, {'$push': {'comment': comment}})
    article = db.article.find_one({'idx': int(article_idx)}, {'_id': False})
    return jsonify({"article": article})
